...
class MaterialModel extends CommonModel{
	...
	//分类下商品
	public function items($cate_id, $limit = 6, $order = "goods_id desc") {
		$cate_ids[$cate_id] = $cate_id;
		$rs = M('material_cate')->where(array('parent_id' => $cate_id))->field('cate_id')->select();
		foreach($rs as $r) {
			$cate_ids[$r['cate_id']] = $r['cate_id'];
		}
		$map = array(
			'cate_id' => array('in', $cate_ids),
		);
		$rs = $this->where($map)->order($order)->limit($limit)->select();
		foreach($rs as $r) {
			$r = $this->_format($r);
			$items[$r['goods_id']] = $r;
		}
