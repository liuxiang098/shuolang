<?php
//wap推站网源码2016年4月21日二次开发，qq738533152
class PassportAction extends CommonAction
{
    ...
    public function sendsms()
    {
        ...
        if ($this->_CONFIG['sms']['dxapi'] == 'dy') {
            $rs = D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_yzm', $mobile, array('sitename' => $this->_CONFIG['site']['sitename'], 'code' => $randstring));
        } else {
            $rs = D('Sms')->sendSms('sms_code', $mobile, array('code' => $randstring));
        }
		if($rs == true) {
			die('1');
		} else {
