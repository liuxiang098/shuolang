...
class MaterialAction extends CommonAction{
    public function _initialize(){
        ...
        $goods = cookie('material');
        $this->assign('cartnum', (int) array_sum($goods));
        //统计商城分类数量代码开始
        $cat = (int) $this->_param('cat');
        $Goods = D('material');
        // $goodscates = D('Materialcate')->fetchAll();
        $goodscates = D('Materialcate')->fetchAll();
        foreach ($goodscates as $key => $v) {
            if ($v['cate_id']) {
                $catids = D('Materialcate')->getChildren($v['cate_id']);
                if (!empty($catids)) {
    ...
    public function index(){
        ...
        $this->assign('cat', $cat);
        $this->assign('nextpage', LinkTo('material/loaddata', array('cat' => $cat, 'order' => $order, 'area' => $area, 'business' => $business, 'cate_id' => $cate_id, 'keyword' => $keyword, 'p' => '0000')));
        $this->display();
    }
    public function main()
    {
        ...
        $this->assign('order', $order);
        $list = D('Material')->order($orderby)->where($map)->limit(0, 10)->select();
    ...
    public function loaddata()
    {
        ...
        $cat = (int) $this->_param('cat');
        if ($cat) {
            $catids = D('Materialcate')->getChildren($cat);
            if (!empty($catids)) {
    ...
    public function buy($goods_id){
        ...
        if (!($detail = D('Material')->find($goods_id))) {
            $this->error('改商品不存在');
        }
        ...
        cookie('goods', $goods);
        header('Location:' . U('Material/cart'));
        die;
    }
    ...
    public function cart(){
        ...
        $cart_goods = D('Material')->itemsByIds($goods_ids);
        $shop_ids = array();
    ...
    public function detail($goods_id) {
        ...
        $recom = D('Material')->where(array('shop_id' => $shop_id,'audit'=>1,'closed'=>1,'goods_id' => array('neq', $goods_id),'end_date'=> array('egt', TODAY)))->select();
        $record = D('Usersgoods');
    ...
    public function pay(){
        ...
        $this->assign('goods', D('Material')->itemsByIds($goods_id));
        $this->assign('shops', D('Shop')->itemsByIds($shop_ids));
        $this->assign('ordergoods', $ordergood);
    ...
    public function paycode2(){
        ...
        if (false == D('Material')->check_goods_id_mum($order_ids)) {//批量检测库存
            $this->fengmiMsg(D('Material')->getError(), U('Material/index'));   
        }
        
        //合并付款暂时无法使用优惠劵
        D('Order')->where(array('order_id' => array('IN', $order_ids)))->save(array('addr_id' => $addr_id));
        /**********************推站网 修复合并付款的时候的系列订单错误问题*****************************/
        ...
        if ($code == 'wait') {
            ...
            $this->fengmiMsg('恭喜您下单成功！', U('Material/goods/index'));
        } else {
