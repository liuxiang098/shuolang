...
class MaterialAction extends CommonAction {
    ...
    public function index() {
        ...
        $this->assign('cates', D('material_cate')->select());
        // var_dump($list);
        // exit;
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if ($goods_id = $obj->add($data)) {
                ...
                $this->baoSuccess('添加成功', U('material/index'));
            }
            $this->baoError('操作失败！');
        } else {
            $this->assign('cates', D('Materialcate')->fetchAll());
            $this->display();
        }
    }
    ...
    private function createCheck() {
        ...
        if (empty($data['cate_id'])) {
         ...
         $Goodscate = D('Materialcate')->where(array('cate_id' => $data['cate_id']))->find();
        ...
        if ($goods_id = (int) $goods_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    ...
                    $this->baoSuccess('操作成功', U('material/index'));
                }
                $this->baoError('操作失败');
            } else {
                $this->assign('detail', $obj->_format($detail));
                // $this->assign('parent_id',D('Goodscate')->getParentsId($detail['cate_id']));
                $this->assign('parent_id',D('Materialcate')->getParentsId($detail['cate_id']));
    ...
    public function delete($goods_id = 0) {
        if (is_numeric($goods_id) && ($goods_id = (int) $goods_id)) {
            ...
            $this->baoSuccess('删除成功！', U('material/index'));
        } else {
    ...
    public function audit($goods_id = 0) {
        if (is_numeric($goods_id) && ($goods_id = (int) $goods_id)) {
            ...
            $this->baoSuccess('审核成功！', U('material/index'));
        } else {
            $goods_id = $this->_post('goods_id', false);
            if (is_array($goods_id)) {
                $obj = D('Material');
                ...
                $this->baoSuccess('审核成功！'.$error.'条失败', U('material/index'));
            }
