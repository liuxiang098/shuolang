<?php
class MaterialcateAction extends CommonAction{
    ...
    public function index(){
        $materialcate = D('Materialcate');
        // $list = $materialcate->fetchAll();
        $list = $materialcate->select();
        // var_dump($list);
        // exit;
    ...
    public function create($parent_id = 0){
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->baoSuccess('添加成功', U('materialcate/index'));
            }
            $this->baoError('操作失败！');
        } else {
    ...
    public function child($parent_id = 0){
        // $datas = D('materialcate')->fetchAll();
        $datas = D('material_cate')->select();
        $str = '';
...
';
        ...
        if ($cate_id = (int) $cate_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $obj->cleanCache();
                    $this->baoSuccess('操作成功', U('materialcate/index'));
                }
                $this->baoError('操作失败');
            } else {
    ...
    public function delete($cate_id = 0){
        if (is_numeric($cate_id) && ($cate_id = (int) $cate_id)) {
            ...
            $obj->delete($cate_id);
            $obj->cleanCache();
            $this->baoSuccess('删除成功！', U('materialcate/index'));
        } else {
    ...
    public function delattr($attr_id){
        ...
        $this->baoSuccess('删除成功！', U('materialcate/setting', array('cate_id' => $detail['cate_id'])));
    }
    public function ajax($cate_id, $goods_id = 0){
        if (!($cate_id = (int) $cate_id)) {
            $this->error('请选择正确的分类');
        }
        if (!($detail = D('materialcate')->find($cate_id))) {
            $this->error('请选择正确的分类');
        }
    ...
    public function setting($cate_id){
        ...
        if ($this->isPost()) {
            ...
            $this->baoSuccess('操作成功！', U('materialcate/setting', array('cate_id' => $cate_id)));
        } else {
            $this->assign('detail', $detail);
            $this->assign('attrs', D('Goodscateattr')->order(array('orderby' => 'asc'))->where(array('cate_id' => $cate_id))->select());
            $this->display();
    ...
    public function update(){
        ...
        $obj->cleanCache();
        $this->baoSuccess('更新成功', U('materialcate/index'));
    }
}